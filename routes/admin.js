...
router.get('/', function(req,res){
    Movies.find({}, function(err, allMovies){
        if(err){
            ...
            res.render('admin.ejs', {allmovies_now: allMovies}); 
        }
    });
});
        ...
        if(err){
            ...
            Movies.find({}, function(err, allmovies){
                if(err){
                    ...
                    res.render('cinema-admin.ejs', {allcinema:allcinema, allmovies:allmovies});
                }
            })
...
router.get('/add-theater/:id_location', function(req, res){
    Theater.findById(req.params.id_location).populate('theater').exec(function(err, foundtheater){
        if(err){
            ...
            res.render('add-theater.ejs', {foundcinema: foundtheater});
...
router.get('/edit/:id', function(req,res){
    Movies.findById(req.params.id, function(err, foundinfo){
        if(err){
            ...
            res.render("edit.ejs", {allmovies_now: foundinfo});
        }
    });
});
//update information
...
//delete
router.delete('/delete/:id', middleware.checkCollectionOwner, function(req, res){
